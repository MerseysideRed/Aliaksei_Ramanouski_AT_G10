...
public class StreamPeopleRunner {
    ...
    public static void main(String[] args) {
        List<Person> filteredPersons = Stream.of(new Person(32, "Коля"), new Person(24, "Оля"), new Person(55, "Вася"), new Person(63, "Маша"))
                .filter(person -> person.getAge() < 60)
                .toList();


        int totalSumOfAge = filteredPersons.stream()
                .sorted((person1, person2) -> person1.getName().compareTo(person2.getName()))
                .flatMap(person -> Stream.of(new Person(person.getAge() + 4, person.getName())))
                .mapToInt(person -> person.getAge())
                .sum();

        double averageAge = (double) totalSumOfAge / filteredPersons.size();
